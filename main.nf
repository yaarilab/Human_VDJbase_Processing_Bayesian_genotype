...
input:
 set val(name),file(airrFile) from g_94_outputFileTSV2_g_75
...
publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /${call}_genotype_report.tsv$/) "genotype_report/$filename"}
input:
 set val(name),file(airrFile) from g_94_outputFileTSV2_g_29
...
output:
 set val(outname),file("${outname}_Final_genotype.tsv")  into g_76_outputFileTSV00
